...
(function($){
	...
	data = {
		...
		getNodesByParamFuzzy: function(setting, nodes, key, value) {
			...
			try{value = value.toLowerCase();}catch(e){}//wanhm 2013-8-6  增加容错
			for (var i = 0, l = nodes.length; i < l; i++) {
		...
		try{	
			...
			if(typeof(setting.search)!="undefined"){
				if(setting.search.enable==true){
					...
					oDIV_.innerHTML = "<div style='background-color:#efeded;border:1px silver solid;margin:1px'>"
								  +"<input size=20 onkeyup='_doTreeFilter(this)'><div onclick='_shift2CheckList()' style='margin:2px;margin-bottom:0px;float:right;cursor:hand' title='view check list'><img src='../../../css/zTreeStyle/img/check_list.gif'></div><div onclick='_shiftTree()' style='margin-left:5px;margin-right:5px;float:right;cursor:hand' title='list->tree'><img src='../../../css/zTreeStyle/img/tree_mode.gif'></div></div>";
				    oDIV_.style.width = domTree_.offsetWidth + "px";//obj.width();
					oDIV_.style.top = _oParent.offsetTop;
  				    //设置  end.............................. 

                    //创建查询结果层 begin___________________________________
					var _divTreeRslt = document.createElement("DIV");
					_divTreeRslt.id = _gstrTreeId + "_search";//树的id加_search的后缀
					
					_divTreeRslt.style.cssText = "position:absolute;overflow:scroll;background-color:white;border:1px white solid;border-top:1px silver solid;border-left:1px #efeded solid;margin-top:30;";
	
					_oParent.appendChild(_divTreeRslt);//if use'document.body.appendChild',chrome会有问题
                    //int 加上"px" ，解决firefox \ chrome 兼容性问题。
					_divTreeRslt.style.width = domTree_.offsetWidth + "px";//obj.width();这样写chrome界面有问题
					_divTreeRslt.style.height = (domTree_.offsetHeight-2) + "px";//obj.height();
					//_divTreeRslt.style.top = domTree_.offsetTop + 27;
					_divTreeRslt.style.left = _oParent.offsetLeft;

					var dimTmp_=document.getElementById(_divTreeRslt.id); //DOM对象
                    var $jSearch=$(dimTmp_); //jQuery对象
					$jSearch.hide();//隐藏查询结果层
					_gTreeSearch = $jSearch;
					//创建... end_______________________________________________
...
var _gTreeTemp;//定义全局的树对象，为jquery对象
var _gTreeSearch;//定义全局的树对象，为jquery对象
var _gstrTreeId;//定义树id，在init方法中初始化。
...
function _getFilterItem(node){
	var strRet_ ="";
    strRet_ = "<div style='cursor:hand;' onclick='_setClickBg()'><input type=checkbox name='_SearchRsltItem' value='" 
	        + node.id + "' >"  + node.name + "</div>";
	return strRet_;
}
//设置查询结果的点击事件
var _pre_rslt_span = null;
function _setClickBg(){
...
function _doTreeFilter(edtFilter){
    ...
    //设置全选过滤bar
	var _sHtmlTmp_="<div style='background-color:buttonface;border:1px silver solid;margin:1px'><input type=checkbox id=_chkAll onclick=_checkAllRslt() value=''>All</div>";
