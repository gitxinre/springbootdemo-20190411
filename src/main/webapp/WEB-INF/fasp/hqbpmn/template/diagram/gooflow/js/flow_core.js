...
function GooFlow(bgDiv,property){
	if (navigator.userAgent.indexOf("MSIE 8.0")>0||navigator.userAgent.indexOf("MSIE 7.0")>0||navigator.userAgent.indexOf("MSIE 6.0")>0)
		GooFlow.prototype.useSVG="";//ie浏览器
	else	GooFlow.prototype.useSVG="1";//非ie浏览器
    //初始化区域图的对象
	this.$id    = bgDiv.attr("id");//定义流程id，所有节点id 均以该id作为前缀，这样保持不同的流程的节点id唯一 wanhm 2014/1/28
	//start:yujf 新增属性
	this.$flowId=null;//工作流ID
	this.$diagramData=null;//工作流属性定义
	this.$bpmnType=null;
	//end:yujf 新增属性
	...
	var w_bg = 800;
	var h_bg = 600;
	if(typeof(property.width)=="undefined")
	    w_bg = $(document).width();
	else 
		w_bg = (property.width||800)-2;
	
	if(typeof(property.height)=="undefined")
	    h_bg = $(document).height();
	else
	    h_bg=(property.height||600)-2;
	
	var width=0,height=0;
	if(property.haveTool){
		width = w_bg - 6 ;//- $(".toolbox").width() - 8;
	    height = 32;
	}
	this.$bgDiv.css({width:width+"px",height:height+"px"});
	this.$toolbox=null;//左侧工具箱
	this.$head=null;//顶部标签及工具栏按钮
	this.$title=null;//流程图的名称
	this.$nowType="cursor";//当前要绘制的对象类型
	this.$lineData={};//定义所有线条数据，json格式
	this.$lineCount=0;//定义划线数量
	this.$nodeData={};//定义所有节点信息数据，json格式
	this.$nodeCount=0;//定义节点数量
	this.$areaData={};//定义区域数据,json格式
	this.$areaCount=0;//定义区域数量
	this.$lineDom={};
	this.$nodeDom={};
	this.$areaDom={};
    ...
    //定义图片节点wanhm2014/2/25
	this.$ImgNodes="ParallelGateway,ExclusiveGateway,Parallelgateway,InclusiveGateway,EventGateway,"
	              +"StartEvent,TimerStartEvent,MessageStartEvent,ErrorStartEvent,EndEvent,ErrorEndEvent,TerminateEndEvent,"
				  +"TimerCatchingEvent,SignalCatchingEvent,MessageCatchingEvent,SignalThrowingEvent,NoneThrowingEvent,";

	if(property.haveToolbar){		
		this.$head=_getJQueryObj(property.toolbar);
		if(property.toolbar){
		    this.$head.on("click",{inthis:this},function(e){
		    	//alert(e.target.id);
				if(!e)e=window.event;
				var tar=e.target;//alert(tar.tagName);
	...
	if(property.haveTool){		
		...
		this.$nowType="cursor";
		//绑定各个按钮的点击事件
		this.$toolbox.on("click",".flatbtn",{inthis:this},function(e){
			if(e.target.id=="") return;
			var _id = e.target.id;
            var _type = _id.substring(3,_id.length);
			if(!e) e=window.event;
			var tar;
			switch(e.target.tagName){
				case "SPAN":return false;
				case "IMG":	tar = e.target.parentNode;break;
				case "DIV":	tar = e.target;
			};
			var type=$(tar).attr("type");
			e.data.inthis.switchToolBtn(type);//设置图形类型，便于当点击画布时，根据该类型进行画图wanhm2014/1/28
			return false;
		});
		this.$editable=true;//只有具有工具栏时可编辑
	}
	width = w_bg ;
	height = h_bg - $("#boxS").height() - $("#myflow").height();
	//设置流程工作区或说是画图区宽高  wanhm 2014/2/21
	$(".GooFlow_work").css({"width":(width)+"px","height":(height)+"px"});
	this.$workArea=$(".GooFlow_work_inner").css({"width":width*1.2+"px","height":height*1.2+"px"})
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){//开始添加节点或是线条wanhm
			if(!e)e=window.event;
			...
			else if(type=="SequenceFlow"||type=="Association"||type=="group")return;
			var X,Y;
			...
			//alert("开始增加节点-> \n 源位图类型为：" + type + "；流程id=" + e.data.inthis.$id);//wanhm2014/1/27
			//alert("添加前所有节点信息是：" + JSON.stringify(e.data.inthis.$nodeData));
			//alert("添加前所有划线信息是：" + JSON.stringify(e.data.inthis.$lineData));
			//alert("添加前area信息是：" + JSON.stringify(e.data.inthis.$areaData));
			var _id =e.data.inthis.$nowType.toLowerCase() + e.data.inthis.$max +"_"+e.data.inthis.$bpmnType;
			var jsonTmp_ = eval("(" + getNodeStr(_id,e.data.inthis.$nowType,X,Y,0,0) + ")" );
			e.data.inthis.addNode(_id,jsonTmp_);
			e.data.inthis.$max++;
			//alert("增加节点后数据："+JSON.stringify(e.data.inthis.$nodeData));//wanhm2014/1/27
	  });
	  //划线时用的绑定
	  this.$workArea.mousemove({inthis:this},function(e){
		if((e.data.inthis.$nowType!="SequenceFlow")&&(e.data.inthis.$nowType!="Association"))	return;
	  ...
	  this.$textArea=$("<textarea bgcolor=red></textarea>");
	  ...
	  this.$lineMove.on("mousedown",{inthis:this},function(e){
		  var lm=$(this);
		  lm.css({"background-color":"#00CD66",opacity:"0.6",filter:"alpha(opacity=60)"});
		  var This=e.data.inthis;
	  ...
	  //当操作某个单元（结点/线/分组块）被添加，触发的方法
	  //格式function(id，type,json)：id是单元的唯一标识ID,type是单元的种类,有"node","line","area"三种取值,json即addNode,addLine或addArea方法的第二个传参json.
	  this.afterItemAdd=null;
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		if(GooFlow.prototype.useSVG!=""){
			...
			defs.appendChild(GooFlow.prototype.getSvgMarker("arrow1","#0686E1"));
	...
	initGroup:function(width,height){//创建类似于泳道的一块东西，只是用来显示，没有实际约束功能  wanhm 2014/1/28
	...
	switchToolBtn:function(type){
		this.$toolbox.children("#"+this.$id+"_btn_"+this.$nowType.split(" ")[0]).attr("class","GooFlow_tool_btn");
		if(this.$nowType=="group"){
			this.$workArea.prepend(this.$group);
			for(var key in this.$areaDom)	this.$areaDom[key].addClass("lock").children("div:eq(1)").css("display","none");
		}
		this.$nowType=type;//标记当前按钮类型，然后，点击画布的时候，系统将根据该类型画相关的图形wanhm2014/1/28
		this.$toolbox.children("#"+this.$id+"_btn_"+type.split(" ")[0]).attr("class","GooFlow_tool_btndown");
		if(this.$nowType=="group"){
			this.blurItem();
	...
	addNode:function(id,json){
		...
		var _ndx = this.$ImgNodes.indexOf(json.type) ;
		if( _ndx < 0){//说明为 非图片对象 wanhm2014/2/25
			if(!json.width||json.width<86)  json.width = 105;
			if(!json.height||json.height<24)  json.height = 30;

			if(!json.top||json.top<0)  json.top=0;
			if(!json.left||json.left<0)  json.left=0;
			if(json.type=="Subprocess"){
				this.$nodeDom[id]=$("<div class='GooFlow_item" + mark + "' type='"+json.type+"' id='" + id + "' style='opacity:"+json.opacity+";filter:alpha(opacity="+(json.opacity*100)+");z-index:"+json.zIndex+";top:" + json.top + "px;left:" + json.left + "px;background-color:#0686dd;border-radius:10px;'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico' valign=top><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
			}else{
				this.$nodeDom[id]=$("<div class='GooFlow_item" + mark + "' type='"+json.type+"' id='" + id + "' style='opacity:"+json.opacity+";filter:alpha(opacity="+(json.opacity*100)+");z-index:"+json.zIndex+";top:" + json.top + "px;left:" + json.left + "px;'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico' valign=top><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
			}
			if(json.type.indexOf(" mix")>-1)	this.$nodeDom[id].addClass("item_mix");
		}
		else{
			//图片对象，固定宽和高  wanhm2014/2/25
			json.width=40;json.height=40;//已经在样式：item_round中定义图片型节点的宽和高，这里定义没用 wanhm2014/2/25
			if(json.type=="StartEvent"){
				this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' type='"+json.type+"' id='"+id+"' style='z-index:"+json.zIndex+";top:"+json.top+"px;left:"+json.left+"px;background-color:#2e8711;'><table cellspacing='0' ><tr><td class='ico'><p class='ico_"+json.type+"'></p></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
			}else if(json.type=="EndEvent"){
				this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' type='"+json.type+"' id='"+id+"' style='z-index:"+json.zIndex+";top:"+json.top+"px;left:"+json.left+"px;background-color:#d91314;'><table cellspacing='0' ><tr><td class='ico'><p class='ico_"+json.type+"'></p></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
			}else{
				this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' type='"+json.type+"' id='"+id+"' style='z-index:"+json.zIndex+";top:"+json.top+"px;left:"+json.left+"px;background-color:#0686DD;'><table cellspacing='0' ><tr><td class='ico'><p class='ico_"+json.type+"'></p></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
			}
		}
		var ua=navigator.userAgent.toLowerCase();
		if(ua.indexOf('msie')!=-1 && ua.indexOf('8.0')!=-1){
			if(json.type=="Subprocess"){
				this.$nodeDom[id].css("filter","progid:DXImageTransform.Microsoft.Alpha(opacity=40)");
			}else{
		...
		if(this.afterItemAdd!=null){
			this.afterItemAdd(id,"node",json);
		}
	},	
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){
			...
			if(e.button==2)return false;
			var This=e.data.inthis;
			if((This.$nowType=="SequenceFlow")||(This.$nowType=="Association")) return;
		...
		//绑定连线时确定初始点（同时显示节点属性，便于修改wanhm2014/2/28）
		this.$workArea.delegate(".GooFlow_item","mousedown",{inthis:this},function(e){
			if(e.button==2)return false;
			var This = e.data.inthis; 

			/********** 显示节点属性 start  wanhm*****************/
			//alert("在右侧显示id="+this.id+";type="+this.type+"的节点属性！");
			var arrtmp = e.data.inthis.$nodeData ;  
			//alert(arrtmp[this.id].name);
			/********** 显示节点属性 end  *****************/
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			//获取划线的空白JSON对象
			var _id="flow"+This.$max+"_"+This.$bpmnType;
			var _jsonTmp = eval( "(" + getLineStr(_id,"sl",lineStart.id,this.id) + ")" );
			if(lineStart)	This.addLine(_id,_jsonTmp);//wanhm2014/4/4  {from:lineStart.id,to:this.id,name:""});
			
			This.$max++;
		});
		//绑定流程节点（图片型节点）双击编辑事件 wanhm2014/2/26
		...
		//绑定流程节点（非图片型节点，如UserTask）双击编辑事件  wanhm2014/2/26
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			var oldTxt = this.innerHTML;//alert(JSON.stringify(e.data.inthis));
			var This = e.data.inthis;//alert(This.$nowType + "==" + This.$id);
			var id = $(this).parents(".GooFlow_item").attr("id");
			var t = getElCoordinate(This.$workArea[0]);//alert(oldTxt);//alert(JSON.stringify(This.$textArea));
			...
			This.$workArea.parent().one("mousedown",function(e){
				...
				This.setName(This.$textArea.data("id"),This.$textArea.val(),"node");
				This.$textArea.val("").removeData("id").hide();
			});//alert(This.$textArea.data("id"));alert(This.$textArea.html());
		});
		//绑定结点的删除功能
			...
			if(jq.prop("tagName")=="DIV"){
				...
				if(GooFlow.prototype.useSVG!=""){
					if(!this.$lineData[this.$focus].marked){
						jq[0].childNodes[1].setAttribute("stroke","#0686E1");
						jq[0].childNodes[1].setAttribute("marker-end","url(#arrow1)");
					}
				}
				else{
					if(!this.$lineData[this.$focus].marked)	jq[0].strokeColor="#0686E1";
				}
	...
	focusItem:function(id,bool){
		var jq=$("#"+id);//alert("当前对象id："+ id );//此处显示本选中图形的属性 wanhm 2014/1/27
		//alert(JSON.stringify(this.getItemInfo(id,"node")));//返回json：{}
	...
	moveNode:function(id,left,top){
		...
		var icoDom = $("#"+id).find("td").eq(0)[0];
		icoDom.style.left = "0px";
		icoDom.style.top = "0px";
	...
	getName:function(id,type){
		var strRet_="";
        if(type=="node"){//如果是结点
			if(!this.$nodeData[id])	strRet_= "";
			strRet_= this.$nodeData[id].name;
		}else if(type=="line"){//如果是线
			if(!this.$lineData[id])	strRet_= "";
			strRet_= this.$lineData[id].name;
		}else if(type=="area"){//如果是分组区域
			if(!this.$areaData[id])	strRet_= "";
			strRet_= this.$areaData[id].name;
		}
		return strRet_;
	},
	//设置结点/连线/分组区域的文字信息
	setName:function(id,name,type){
		...
		if(type=="node"){//如果是结点
			...
			if(this.$nodeData[id].type=="StartEvent"||this.$nodeData[id].type=="EndEvent"||this.$nodeData[id].type=="ExclusiveGateway"||this.$nodeData[id].type=="Parallelgateway"||this.$nodeData[id].type=="InclusiveGateway"){
	...
	//设置左上角流程图的名称
	setTitle:function(text){//alert("settitle:"+text);//wanhm2014/1/27
		this.$title=text;
		if(this.$head)	this.$head.children("label").attr("title",text).text(text);
	},
	//载入一组数据
	loadData:function(data){
		var bIfEditable = this.$editable;
		this.$editable=false;
		//start:yujf 新增工作流ID属性，记录工作流ID
		if(data.id){
			this.$flowId=data.id;
		}
		if(data.diagram.bpmn.categroy){
			this.$bpmnType=data.diagram.bpmn.categroy;
		}
		
		//end:yujf 新增工作流ID属性，记录工作流ID
		...
		//start：yujf	记录工作流属性
		this.$diagramData=$.extend({},data.diagram);
		//end：yujf	记录工作流属性
		this.$editable = bIfEditable;
		this.$deletedItem={};
	},
	//用AJAX方式，远程读取一组数据
	...
	transNewId:function(oldId,newId,type){
		...
		switch(type){
			...
			if(this.$nodeData[oldId]){
				...
				//start:yujf 修改nodeDom模型中的ID 
				tmp=this.$nodeDom[oldId];
				delete this.$nodeDom[oldId];
				tmp.attr("id",newId);
				this.$nodeDom[newId]=tmp;
				
				this.$focus=newId;//重新制定选中节点ID
				//end:yujf 修改nodeDom模型中的ID 
			}
			break;
			case "line":
			if(this.$lineData[oldId]){
				...
				//start:yujf 修改lineDom模型中的ID 
				tmp=this.$lineDom[oldId];
				delete this.$lineDom[oldId];
				$(tmp).attr("id",newId);
				this.$lineDom[newId]=tmp;
				
				this.$focus=newId;//重新制定选中连接线ID
				//end:yujf 修改lineDom模型中的ID 
			}
			break;
			case "area":
	...
	drawLine:function(id,sp,ep,mark,dash){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			if(mark){
				...
				path.setAttribute("stroke","#0686E1");
			...
			if(id!=""&&id!="GooFlow_tmp_line"){
				...
				text.className="hq_flow_line_text";
				//text.textContent=id;
				line.appendChild(text);
				var x=(ep[0]+sp[0])/2;
				var y=(ep[1]+sp[1])/2;
				text.setAttribute("class","hq_flow_line_text");
			...
			line=document.createElement("v:polyline");window.document.body.appendChild(line);
			if(id!="")	line.id=id;
	...
	addLine:function(id,json){
		...
		this.$lineData[id]=json;//wanhm2014/4/4  {};
		...
		if(GooFlow.prototype.useSVG==""){
			...
			//this.$lineDom[id].childNodes[2].hide();
		}	
		++this.$lineCount;
		if(this.$editable){
			this.$lineData[id].alt=true;
	...
	resetLines:function(id,node){//alert(JSON.stringify(node));
	...
	markItem:function(id,type,mark){
		if(type=="historyNode"){
			...
			if(mark)	this.$nodeDom[id].addClass("item_markRed");
			else	this.$nodeDom[id].removeClass("item_markRed");
			...
			if(GooFlow.prototype.useSVG!=""){
				if(mark){
					...
					this.$lineDom[id].childNodes[1].setAttribute("stroke","#0686E1");
					this.$lineDom[id].childNodes[1].setAttribute("marker-end","url(#arrow1)");
				}
			}else{
				if(mark)	this.$lineDom[id].strokeColor="#ff3300";
				else	this.$lineDom[id].strokeColor="#0686E1";
			}
		}else if(type=="currentNode"){
			if(!this.$nodeData[id])	return;
			if(this.onItemMark!=null&&!this.onItemMark(id,"node",mark))	return;
			this.$nodeData[id].marked=mark||false;
			if(mark)	this.$nodeDom[id].addClass("item_markGreen");
			else	this.$nodeDom[id].removeClass("item_markGreen");
		}
		if(this.$undoStatck){
	...
	reinitSize:function(width,height){
		...
		if(this.$toolbox!=null){
			this.$toolbox.css({height:h-headHeight-hack+"px"});
		}
		w-=39;
...
//根据id获取jquery对象
function _getJQueryObj(id){
    var objTmp_ = document.getElementById(id); //DOM对象
    return $(objTmp_); //jQuery对象
}
//根据jquery对象，获取dom对象
function _getDOM(jq){
    var $v = jq;
...
parameters   :   
                    id:
			   imgtype:图元类型，如UserType,sl,StartEvent等		
			      left:
				   top:
return value : string    返回结果格式如:{type:"",left:111,width:222,MainConfig:{ID:"",candidateUser:""}}
************************************/
function getNodeStr(id,imgtype,left,top,width,height){
    var strRet_="";
	strRet_ = "{type:\""+imgtype+"\",left:"+left+",top:"+top+",width:" + width + ",height:" + height +",name:\""+id+"\",";
    
	strRet_ += _getEmptyJSON(imgtype,id);
	strRet_ += "}";
	return strRet_;
}
...
/************************************
Method       : getLineStr
purpose      : 获取流转条件初始化属性对象
parameters   :   
                    id:
			   imgtype:图元类型，如UserType,sl,StartEvent等		
return value : string    返回结果格式如:{type:"sl",from:"demo_node_1",to:"demo_node_2",name:"送部门经理审批",marked:true}
************************************/
function getLineStr(id,imgtype,from,to){
    var strRet_="";
	strRet_ = "{type:\"" + imgtype + "\",from:\"" + from + "\",to:\"" + to + "\",name:\"\",marked:true,";
...
/************************************
Method       : _getEmptyJSON
purpose      : 根据图元类型获取该图元对应的空白json对象。
parameters   :                    
			   imgtype:图元类型，如UserType,sl,StartEvent等		
			        id:
return value : string    返回结果格式如:node_1:{type:"",left:111,width:222,MainConfig:{ID:"",candidateUser:""}}
************************************/
function _getEmptyJSON(imgtype,id){
	var strRet_="";

    var jsonObj;
	if(imgtype=="sl" || imgtype=="lr" || imgtype=="tb")
		jsonObj= eval("FlowDict.SequenceFlow");
	else
		jsonObj= eval("FlowDict." + imgtype);

    for(attr in jsonObj){
		var objTmp_ = jsonObj[attr];
		if(typeof(objTmp_)=="object"){
			strRet_ += attr + ":" + "{";
			for(subattr in objTmp_){
				if(subattr.toUpperCase()=="ID")
				    strRet_ += subattr + ":" + "\""+id+"\",";
				else
					strRet_ += subattr + ":" + "\"\",";
			}
			strRet_ = strRet_.substring(0,strRet_.length-1);
			strRet_ += "},";
		}		
	}
	strRet_ = strRet_.substring(0,strRet_.length-1);
